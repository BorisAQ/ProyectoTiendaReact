...
import { getUsers, getProductosFromDB } from '../../services/database';
...
class Login extends React.Component{
  ...
  handleSubmit(e){
    e.preventDefault();
    const { addLogin , setProductos} = this.props;
        ...
        fetch (getUsers()).then(
          ...
          data=>{
            if( data.some(e => e.usuario === this.state.username && String(e.password) === String(this.state.password))){
              ...
              addLogin({usuario:this.state.username, loggeado:true})
              getProductosFromDB (setProductos)
            }else{
